...
class ImageUndistorter:
    ...
    def _apply_camera_distortion(self, u: np.ndarray,
                                 v: np.ndarray) -> np.ndarray:
        """Converted from instant-ngp/include/neural-graphics-primitives/
        common_device.cuh.
        """
        k_1, k_2, p_1, p_2 = self._D
    ...
    def _iterative_camera_undistortion(
        ...
        for _ in range(kNumIterations):
            step0 = max(sys.float_info.epsilon, np.abs(kRelStepSize * x[0]))
            step1 = max(sys.float_info.epsilon, np.abs(kRelStepSize * x[1]))
            dx = self._apply_camera_distortion(x[0], x[1])
            dx_0b = self._apply_camera_distortion(x[0] - step0, x[1])
            dx_0f = self._apply_camera_distortion(x[0] + step0, x[1])
            dx_1b = self._apply_camera_distortion(x[0], x[1] - step1)
            dx_1f = self._apply_camera_distortion(x[0], x[1] + step1)
    ...
    def _compute_source_to_target_mapping(self) -> None:
        ...
        with multiprocessing.Pool(processes=8) as pool:
            res = np.array(
                pool.starmap(
                    self._iterative_camera_undistortion,
                    [(u, v)
                     for (u, v) in np.stack(
                         np.meshgrid(np.arange(self._W), np.arange(
        ...
        self._u_transformed_rounded = u_transformed.round().astype(int)
        self._v_transformed_rounded = v_transformed.round().astype(int)

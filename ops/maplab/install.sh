...
sudo apt install -y autotools-dev ccache doxygen dh-autoreconf git liblapack-dev libblas-dev libgtest-dev libreadline-dev libssh2-1-dev pylint clang-format-3.9 python-autopep8 python-catkin-tools python-pip python-git python-setuptools python-termcolor python-wstool libatlas3-base libv4l-dev --yes
...
# then
#   git clone https://github.com/ethz-asl/maplab_experimental.git --recursive
# else
#   git clone https://github.com/ethz-asl/maplab.git --recursive -b develop
# fi

echo -e "\e[92m\e[1mBuilding maplab...\e[39m\e[0m"
catkin build maplab rovioli -j2 --mem-limit 50

echo "\e[92m\e[1mDone in $SECONDS seconds."

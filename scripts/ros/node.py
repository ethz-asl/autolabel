...
def to_pose(pose_stamped):
    ...
    T_CW[:3, :3] = Rotation.from_quat([
        ...
        pose_stamped.pose.orientation.w]).as_matrix()
    T_CW[:3, 3] = [
...
class TrainingLoop:
    ...
    def __init__(self, bridge):
        ...
        #         [10, 25, 50], gamma=0.1)
        scheduler = lambda optimizer: optim.lr_scheduler.ConstantLR(optimizer, lr)
        ...
        opt = Namespace(rand_pose=-1,
                        ...
                        semantic_weight=0.0,
                        feature_weight=0.0)
        self.model = model_utils.create_model(min_bounds, max_bounds, 2,
                                         opt)
        ...
        camera = Camera(np.array([[513.104, 0.0, 321.532],
            [0.0, 513.104, 184.124],
            [0., 0., 1.]]), (640, 360))
        self.dataset = DynamicDataset(4096, camera)
        ...
        self.pixel_indices = np.arange(self.render_resolution[0] * self.render_resolution[1])
        self.odometry_pose = None
        self.image_pub = rospy.Publisher('/autolabel/image', Image, queue_size=1)
        self.feature_pub = rospy.Publisher('/autolabel/features', Image, queue_size=1)
    ...
    def render_frame(self):
        ...
        origin = T_WC[:3, 3]
        origins = np.broadcast_to(origin, (resolution[1], resolution[0], 3)).astype(np.float32)
        R_WC = np.ascontiguousarray(T_WC[:3, :3])
        fx = 205.
        fy = 205.
        cx = 256. / 2.
        cy = 192. / 2.
        directions = _compute_direction(R_WC, self.pixel_indices, resolution[0], fx, fy, cx, cy, False)
        directions = directions.reshape((self.render_resolution[1], self.render_resolution[0], 3))
        ...
        with torch.inference_mode():
            with torch.cuda.amp.autocast(enabled=True):
                outputs = self.model.render(rays_o, rays_d, staged=True, perturb=False, max_ray_batch=2048)
        image = outputs['image']
...
class AutolabelNode:
    ...
    def __init__(self, flags):
        ...
        self.image_sub = rospy.Subscriber('/slam/rgb', Image,
                                          self.image_callback, queue_size=20)
        self.depth_sub = rospy.Subscriber('/slam/depth', Image,
                                          self.depth_callback, queue_size=20)
        self.odometry_sub = rospy.Subscriber('/slam/odometry', PoseStamped, self.odometry_callback)
        self.keyframe_sub = rospy.Subscriber('/slam/keyframe', PoseStamped, self.keyframe_callback, queue_size=20)
        self.depth_messages = {}
        self.rgb_messages = {}
        self.pose_messages = {}
    ...
    def _check_tuple(self, seq_num):
        if (seq_num in self.depth_messages and seq_num in self.rgb_messages and seq_num in self.pose_messages):
            rgb = self.rgb_messages[seq_num]
            depth = self.depth_messages[seq_num]
            pose = self.pose_messages[seq_num]
            self.image_tuple(rgb, depth, pose)
            del self.rgb_messages[seq_num]
            del self.depth_messages[seq_num]
            del self.pose_messages[seq_num]

    def image_tuple(self, depth_msg, image_msg, pose_msg):
        T_CW = to_pose(pose_msg)
        if np.abs(depth_msg.header.stamp.to_sec() - image_msg.header.stamp.to_sec()) > self.sync_threshold:
